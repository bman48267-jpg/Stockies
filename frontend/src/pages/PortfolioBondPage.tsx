...
export function PortfolioBondPage() {
  ...
  const {
    ...
    queryKey: ['portfolio-transactions', 'bond'],
    queryFn: () => getTransactions('bond'),
  });
  ...
  const handleDeleteHolding = async (symbol: string, assetType: 'bond') => {
    if (
      !window.confirm(
  ...
  const holdings = summary?.holdings.filter((h) => h.asset_type === 'bond') || [];
  const listTxns = txns || [];

  const investedVal = summary?.allocation.bond_invested ?? 0;
  const currentVal = summary?.allocation.bond_value ?? 0;
  ...
  const COLORS = ['#14b8a6', '#0d9488', '#2dd4bf', '#115e59', '#99f6e4', '#134e4a'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center sm:flex-row flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted">Fixed income bonds & sovereign securities</span>
        </div>
        <button
          onClick={() => setModalOpen(true)}
          className="btn-primary px-5 py-2.5 rounded-xl flex items-center gap-1.5 active:scale-[0.98] transition-all bg-teal-600 hover:bg-teal-500 text-white font-bold"
        >
          <Plus size={16} />
          Add Bond Purchase
        </button>
      ...
      {listTxns.length === 0 ? (
        ...
        >
          <Award size={44} className="mx-auto text-teal-500 mb-4 opacity-75 animate-bounce" />
          <h3 className="text-lg font-bold">No Bond Holdings Tracked</h3>
          <p className="text-sm text-secondary mt-1 max-w-sm mx-auto">
            Track sovereign gold bonds (SGB), structural bonds, NCDs, and corporate debt instruments.
          </p>
          <button onClick={() => setModalOpen(true)} className="btn-primary mt-4 bg-teal-600 hover:bg-teal-500 text-white font-bold">
            <Plus size={16} />
            Register Bond Holdings
          </button>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              ...
              <p className="text-xs text-muted mt-1.5">Across diverse issuances</p>
            </div>
          </div>
          ...
          {holdings.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                <h4 className="text-xs font-bold uppercase tracking-wider text-muted font-sans">
                  Bond Value Accrual / Yield Growth
                </h4>
                <div className="h-[21rem]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={returnsData} margin={{ top: 20, right: 15, left: 10, bottom: 20 }}>
                      <defs>
                        <linearGradient id="bondPnlGlow" x1="0" y1="0" x2="0" y2="1">
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                <thead>
                  <tr>
                    <th>Bond Instrument Target Code</th>
                    <th>Identifier</th>
                    <th className="text-right">Quantity</th>
                    <th className="text-right">Purchase Unit Cost</th>
                    <th className="text-right">Live / Estim NAV</th>
                    <th className="text-right">Invested value</th>
                    <th className="text-right">Current value</th>
                    <th className="text-right">Gains / Total returns</th>
