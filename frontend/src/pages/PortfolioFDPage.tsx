...
export function PortfolioFDPage() {
  ...
  const {
    ...
    queryKey: ['portfolio-transactions', 'fixed_deposit'],
    queryFn: () => getTransactions('fixed_deposit'),
  });
  ...
  const handleDeleteHolding = async (symbol: string) => {
    ...
    try {
      const records = await getTransactions('fixed_deposit', symbol);
      if (records.length === 0) {
      ...
      queryClient.invalidateQueries({ queryKey: ['portfolio-transactions', 'fixed_deposit'] });
      handleRefetch();
    } catch (err) {
      console.error(err);
  ...
  const holdings = summary?.holdings.filter((h) => h.asset_type === 'fixed_deposit') || [];
  const listTxns = txns || [];

  const investedVal = summary?.allocation.fixed_deposit_invested ?? 0;
  const currentVal = summary?.allocation.fixed_deposit_value ?? 0;
  ...
  const returnsData = holdings.map((h) => ({
    symbol: h.symbol,
    name: h.name,
    'Returns (₹)': h.unrealized_pnl,
  })).sort((a, b) => b['Returns (₹)'] - a['Returns (₹)']);

  const COLORS = ['#eab308', '#ca8a04', '#facc15', '#854d0e', '#fef08a', '#713f12'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center sm:flex-row flex-col gap-3">
        <div className="flex items-center gap-2">
          <button
            ...
            title="Refresh Fixed Deposits"
          >
            <RefreshCw size={15} />
          </button>
          <span className="text-xs text-muted">Secure locked fixed-term deposits</span>
        </div>
        <button
          onClick={() => setModalOpen(true)}
          className="btn-primary px-5 py-2.5 rounded-xl flex items-center gap-1.5 active:scale-[0.98] transition-all bg-yellow-600 hover:bg-yellow-500 text-black font-bold"
        >
          <Plus size={16} />
          New Fixed Deposit
        </button>
      ...
      {listTxns.length === 0 ? (
        ...
        >
          <Lock size={44} className="mx-auto text-yellow-500 mb-4 opacity-75 animate-bounce" />
          <h3 className="text-lg font-bold">No Fixed Deposits Registered</h3>
          <p className="text-sm text-secondary mt-1 max-w-sm mx-auto">
            Lock in guaranteed returns. Track details, interest accruals, and maturity values of your bank and corporate FDs.
          </p>
          <button onClick={() => setModalOpen(true)} className="btn-primary mt-4 bg-yellow-600 hover:bg-yellow-500 text-black font-bold">
            <Plus size={16} />
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              ...
              <h3 className="text-2xl font-extrabold text-yellow-400 font-mono">
            ...
            >
              ...
              <p className={`text-xs font-semibold mt-1.5 ${signClass(pnlPercent)}`}>
                {formatPercent(pnlPercent)} gains
              </p>
            </div>
            ...
            >
              ...
              <p className="text-xs text-muted mt-1.5">Across banking accounts</p>
            </div>
          </div>
          ...
          {holdings.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                <h4 className="text-xs font-bold uppercase tracking-wider text-muted font-sans">
                  Fixed Deposit P&L Accruals
                </h4>
                <div className="h-[21rem]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={returnsData} margin={{ top: 20, right: 15, left: 10, bottom: 20 }}>
                      <defs>
                        <linearGradient id="fdPnlGlow" x1="0" y1="0" x2="0" y2="1">
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                <thead>
                  <tr>
                    <th>FD Scheme Name</th>
                    <th>Identifier</th>
                    <th className="text-right">Quantity</th>
                    <th className="text-right">Principal Unit Cost</th>
                    <th className="text-right">Accrued Unit Value</th>
                    <th className="text-right">Principal Sum</th>
                    <th className="text-right">Maturity Projection</th>
                    ...
                    <tr key={h.symbol}>
                      ...
                      <td className="text-center">
                        <button
                          ...
                          title="Remove Fixed Deposit holding"
                        >
      ...
      <TransactionModal
        ...
        defaultAssetType="fixed_deposit"
      />
    </div>
  );
}
